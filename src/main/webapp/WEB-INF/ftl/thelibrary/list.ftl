            ...
            <form method="post" action="" id="formId" class="form-inline">
                ...
                <#if page?exists && page.list?size gt 0 >
                    <#list page.list as it>
                        <tr>
                          <td>${it.sid?default('未设置')}</td>
                            <td>${(it.sdate?string("yyyy/MM/dd"))?default('未设置')}</td>
                            <td>${it.uMajor.name?default('未设置')}</td>
                            <td>${it.snumber?default('未设置')}</td>
                            <td>${it.sname?default('未设置')}</td>
                            <td>${it.edition?default('未设置')}</td>

                            <td>
                                   <a href="${basePath}/thelibrary/list.shtml?">入库记录</a>
                                   <a href="${basePath}/stock/list.shtml">出库记录</a>
                           </td>
