            ...
            <form method="post" action="" id="formId" class="form-inline">
                ...
                <#if page?exists && page.list?size gt 0 >
                    <#list page.list as it>
                        <tr>
                            <td><input value="${it.uid}" check='box' type="checkbox"/></td>
                            ...
                            <td>${(it.lastLoginTime?string('yyyy-MM-dd HH:mm'))?default('')}</td>
                            <td>
                        <@shiro.hasPermission name="/member/forbidUserById.shtml">
                            ${(it.status==1)?string('<i class="glyphicon glyphicon-eye-close"></i>&nbsp;','<i class="glyphicon glyphicon-eye-open"></i>&nbsp;')}
                                <a href="javascript:forbidUserById(${(it.status==1)?string(0,1)},${it.uid})">
                                ${(it.status==1)?string('禁止登录','激活登录')}
                                </a>
                        </@shiro.hasPermission>
                        <@shiro.hasPermission name="/member/deleteUserById.shtml">
                                <a href="javascript:_delete([${it.uid}]);">删除</a>
                        </@shiro.hasPermission>
                            </td>
                        </tr>
                    </#list>
