package com.hp.core.shiro.session;


import com.hp.common.model.UUser;
import com.hp.common.utils.LoggerUtils;
import com.hp.common.utils.StringUtils;
import com.hp.core.shiro.CustomShiroSessionDAO;
import com.hp.user.bo.UserOnlineBo;
...
public class CustomSessionManager {
	 ...
	 * session status 
	 */
	public static final String SESSION_STATUS ="hp-online-status";
		...
		for (Session session : sessions) {
			...
			if(null != obj && obj instanceof SimplePrincipalCollection){
				...
				if(null != obj && obj instanceof UUser){
					...
					//比较用户ID，符合即加入集合
					if(null != user && idset.contains(user.getUid())){
						list.add(spc);
					}
				}
			}
